...
internal class DirectoryCommand(IOptions<ServerStartOptions> optionsProvider, ILogger<DirectoryCommand> logger) : ICommandHandler<DirectoryCommand.CommonDirectory>
{
    ...
    public Task Execute(ICommandContext context, [Description("Common name of the directory to open")] CommonDirectory commonDirectory = CommonDirectory.SAVE)
    {
        ...
        logger.ZLogInformation($"Opening directory {path.ToSensitive():@Path}");
    ...
    private string GetPathFromCommonDirectory(CommonDirectory commonDir = CommonDirectory.SELF) =>
        commonDir switch
        {
            ...
            CommonDirectory.GAME => optionsProvider.Value.GameInstallPath,
